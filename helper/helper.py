...
def __parsingProfileComet__(resp: requests.Response) -> tuple[list, str, bool, bool]:
    edge_list = []
    resps = resp.text.split('\r\n', -1)
    temp_cursor = ""
    is_up_to_time = True
                ...
                for edge in check['node']['timeline_list_feed_units']['edges']:
                    try:
                        ...
                        if edge["creation_time"] != 0:
                            is_up_to_time, arrive_first_catch_time = Auxiliary.dateCompare(edge["creation_time"])
                            if is_up_to_time:
                                edge_list.append(edge)
                    except Exception as e:
                        print(traceback.format_exc())
...
def __parsingCometModern__(resp: requests.Response) -> tuple[list, str, bool, bool]:
    edge_list = []
    resp = json.loads(resp.text.split('\r\n', -1)[0])
    temp_cursor = ""
    is_up_to_time = True
    ...
    while True:
        ...
        try:
            ...
            if reqName == 'ProfileCometTimelineFeedRefetchQuery':
                edge_list, cursor_now, is_up_to_time, arrive_first_catch_time = __parsingProfileComet__(resp)
            elif reqName == 'CometModernPageFeedPaginationQuery':
                edge_list, cursor_now, is_up_to_time, arrive_first_catch_time = __parsingCometModern__(resp)
...
def fetchEigenvaluesAndID(func, customDriver: Union[webDriver.postsDriver, webDriver.feedbackDriver, webDriver.friendzoneDriver], pageURL, errString, checkOption=2) -> tuple[str, str, str, bool]:
