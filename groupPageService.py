...
def runGroupPage(jsonArrayDataSub, fbDTSG, processNum) -> str:
    ...
    posts_driver = webDriver.postsDriver(
        driver=None, options=None, isLogin=False)
    posts_driver.setOptions(
        needHeadless=configSetting.need_headless, needImage=False)
    posts_driver.driverInitialize()

    feedback_driver = webDriver.feedbackDriver(
        driver=posts_driver.driver, options=None, isLogin=True)
    feedback_driver.setOptions(
        needHeadless=configSetting.need_headless, needImage=False)

    screenshot_driver = webDriver.screenshotDriver(
        driver=posts_driver.driver, options=None, isLogin=True)
    screenshot_driver.setOptions(
        needHeadless=configSetting.need_headless, needImage=False)
    ...
    for target_url, target_name in zip(target_urls, target_names):
        ...
        feedback_id_list, story_id_list = parser.buildCollectData(
            post_list, target_name, screenshotDriver=screenshot_driver, is_use_for_group=True)
        ...
        #                           pageID=group_id, screenshotDriver=screenshot_driver)
...
if __name__ == '__main__':
    ...
    # 按行程的數量平分工作量
    target_url_split = Auxiliary.split(
        configSetting.json_array_data['targetURL'], process_worker)
    target_name_split = Auxiliary.split(
        configSetting.json_array_data['targetName'], process_worker)
    args_list = []
    result_list = []
    process_futures = []
    ...
    with ProcessPoolExecutor(max_workers=process_worker) as executor:
        ...
        for i in range(len(args_list)):
            ...
            process_future = executor.submit(
                runGroupPage, args_list[i], fb_dtsg, i)
