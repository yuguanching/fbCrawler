...
def runGroupPage(jsonArrayDataSub, fbDTSG, feedback_docid,  processNum) -> str:
    ...
    for target_url, target_name in zip(target_urls, target_names):
        ...
        post_list = crawlRequests.crawlGroupPosts(pageURL=target_url, groupPageID=group_id, groupDocID=group_page_docid, reqName=page_req_name,
                                                    processNum=processNum, targetName=target_name)
        feedback_id_list, story_id_list, article_id_list = parser.buildCollectData(
            post_list, target_name, group_id, screenshotDriver=screenshot_driver, is_use_for_group=True)
...
if __name__ == '__main__':
    ...
    with ProcessPoolExecutor(max_workers=process_worker) as executor:
        ...
        for i in range(len(args_list)):
            ...
            process_future = executor.submit(runGroupPage, args_list[i], fb_dtsg, feedback_docid,  i)
