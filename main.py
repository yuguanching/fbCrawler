...
def runFansPage(jsonArrayData, fbDTSG, processNum) -> str:
    ...
    feedback_driver.setOptions(needHeadless=True, needImage=False)
    ...
    for target_url,target_name in zip(target_urls, target_names):
        ...
        page_id, page_docid, page_req_name, is_been_banned = helper.fetchEigenvaluesAndID(func=helper.__getPageID__, customDriver=posts_driver, errString="未能取得文章的docid 或 pageid,嘗試換其他帳號試試", pageURL=target_url, jsonArrayData=jsonArrayData, checkOption=2)
        if is_been_banned :
            writer.writeLogToFile(f"目標{target_name}已被臉書封鎖,留下紀錄待處理")
            continue
        
        post_list = helper.crawlPagePosts(pageURL=target_url, pageID=page_id, docID=page_docid, reqName=page_req_name, jsonArrayData=jsonArrayData, proxyIpList=proxy_ip_list, processNum=processNum, targetName=target_name)
        ...
        _, feedback_docid, req_name, is_been_banned = helper.fetchEigenvaluesAndID(func=helper.__getDocIDFeedback__, customDriver=feedback_driver, errString="未能取得分享的docid,嘗試換其他帳號試試", pageURL=target_url, jsonArrayData=jsonArrayData, checkOption=1)
        if is_been_banned :
            writer.writeLogToFile(f"目標{target_name}已被臉書封鎖,留下紀錄待處理")
            continue
        ...
        if feedback_docid != "" :
            if len(feedback_id_list)!=0: 
                ...
                with ThreadPoolExecutor(max_workers=thread_workers) as executor:
                    ...
                    for feedback_id in feedback_id_list:
                        future = executor.submit(helper.crawlFeedback, target_url, feedback_id, feedback_docid, posts_count, req_name, fbDTSG, target_name, processNum, q_data, q_signal)
            ...
            parser.buildDataParse(feedback_data_list, target_name, jsonArrayData)
...
if __name__ == '__main__':
    ...
    # 按行程的數量平分工作量
    target_url_split = Auxiliary.split(jsonArrayData['targetURL'], task_num)
    target_name_split = Auxiliary.split(jsonArrayData['targetName'], task_num)
    args_list = []
    result_list = []
